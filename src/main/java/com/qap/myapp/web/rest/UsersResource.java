...
import com.qap.myapp.repository.UsersRepository;
...
/**
 * REST controller for managing {@link com.qap.myapp.domain.Users}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class UsersResource {
     ...
     */
    @GetMapping("/users")
    public List<Users> getAllUsers(@RequestParam(required = false) String filter) {
        if ("questionlikes-is-null".equals(filter)) {
            log.debug("REST request to get all Userss where questionLikes is null");
            return StreamSupport
                .stream(usersRepository.findAll().spliterator(), false)
                .filter(users -> users.getQuestionLikes() == null)
                .collect(Collectors.toList());
        }
        if ("answerlikes-is-null".equals(filter)) {
            log.debug("REST request to get all Userss where answerLikes is null");
            return StreamSupport
                .stream(usersRepository.findAll().spliterator(), false)
                .filter(users -> users.getAnswerLikes() == null)
                .collect(Collectors.toList());
        }
